...
public class ATNode {    //攻击防御树攻击节点类型
	private static int nums = 0;        //静态数据成员——计数（攻击防御树节点的总数）
	...
	public enum RefinementType {       //枚举的细化类型
	...
	public String getExp1() {
		if (!children.isEmpty()) {         //如果有孩子节点
			if (refinementType == RefinementType.CONJUNCTIVE) {     //如果有孩子节点 且 如果逻辑与
				Iterator<ATNode> it = children.iterator();   //children的迭代器it
				ATNode node1 = null;
				ATNode node2 = null;
				String s1 = null;
				String s2 = null;
				...
				while (it.hasNext()) {        //it迭代器进行遍历
					StringBuffer sb = new StringBuffer();    //StringBuffer：字符串缓冲区
					if (node1 == null) {
						node1 = (ATNode) it.next();
						s1 = node1.getExp1();    //递归   树形结构下一层
						if (it.hasNext()) {
					...
					String[] sa1 = s1.split(" ");  //将一个字符串分割为字符串数组
					if (!(s2 == null)) {      //如果s2非空
						String[] sa2 = s2.split(" ");  //将一个字符串分割为字符串数组
						...
						temp = sb.substring(0, sb.length() - 1);    //起始索引到结束索引(不包括结束索引对应的元素)
					} else
						temp = s1;
				}
				System.out.println(temp);
				return temp;    //如果有孩子节点 且 如果逻辑与 则返回这个
				
			} else {                //如果有孩子节点 且 如果逻辑或
				...
				return sb.substring(0, sb.length() - 1);  //如果有孩子节点 且 如果逻辑或 则返回这个
			}
		}
		// System.out.println(this.label);
		return this.label;         //如果没有孩子节点
	}
